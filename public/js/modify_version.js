...
$(function() {
	var publish_branch = [];
	$(".checked-btn[data-checked='true']").each(function(){
		publish_branch.push($(this).next().text());
	})
	...
	$('.publish-sure').unbind().click(function(){
		...
		if(publish_version == '' ||  publish_explain == '' || publish_time == '' || publish_branch.length == 0){
			...
			var req = {
			    ...
			    id: $('.publish-sure').attr("data-id")
			}
			$.get("/modifyVersionInfo", req, function(data) {
				window.location.href = "/versionList";
			})
		}
	});
	$('.publish-remove').unbind().click(function(){
		var req = {
		    id: $('.publish-remove').attr("data-id")
		}
		$.get("/removeVersion", req, function(data) {
			window.location.href = "/versionList";
		})
	})
})
